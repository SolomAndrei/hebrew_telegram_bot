...
- [x] Добавлен backend endpoint `GET /api/articles/:id` с inline-токенами для чтения
...
- [x] Bottom Sheet показывает перевод, транскрипцию, лемму и форму слова с огласовками
- [x] Перевод слова берется из уже загруженного текста без отдельного API-запроса
- [x] Добавлен переключатель отображения огласовок в Mini App
- [x] Кнопка завершения чтения показывает spinner, отправляет статистику без учета времени чтения и закрывает Mini App после успешного ответа
- [x] Bot reply открывает статью через real Telegram Mini App button при настроенном Mini App URL

### Inline-переводы и огласовки

- [x] Слова приходят сразу вместе с переводом в ответе `GET /api/articles/:id`
- [x] Отдельный клиентский запрос перевода слова удален из UX
- [x] Перевод привязан к конкретному вхождению слова в тексте, а не к глобальному словарю по строке слова
- [x] Из клиентского ответа статьи убран `title`
- [x] Из клиентского перевода убраны лишние поля: часть речи, причина выбора базовой формы, альтернативы
- [x] Транскрипция показывается в Mini App, но вычисляется алгоритмически из `pointedText`, а не генерируется LLM
- [x] Для слов генерируется форма с огласовками `pointedText`
- [x] На клиенте можно включить или выключить отображение огласовок
- [x] Для хранения токенов статьи добавлена JSONB-колонка `articles.reading_tokens`
- [ ] Замена LLM-перевода слов библиотекой не реализована: текущая реализация оставляет LLM, потому что перевод должен учитывать контекст предложения

### Словарь и прогресс

- [x] При клике lemma слова собирается на клиенте и сохраняется в `user_words` при завершении чтения
...
- [x] Клик по слову показывает уже загруженный контекстный перевод без отдельного API-запроса
...
- `AdaptationModule`: provider-neutral LLM adapter на Vercel AI SDK, schema-based JSON output, prompt адаптации, validator prompt, enrichment prompt для inline-токенов с переводом и огласовками, retry до 2 повторных генераций, typed fallback если адаптация не прошла проверку.
- `ArticlesModule`: сохранение адаптированных статей и `reading_tokens` в таблицу `articles`, защищённое чтение статьи по `id` для текущего пользователя.
...
- `mini-app/`: Vite / React / Tailwind frontend skeleton, Telegram initData header, client API для `GET /api/me` и `GET /api/articles/:id`, RTL экран чтения, локальный Bottom Sheet перевода по inline-токенам, переключатель огласовок, spinner и закрытие Mini App после успешного завершения чтения.
...
Система берет реальные новости или произвольные тексты, адаптирует их под уровень пользователя через LLM и предоставляет удобный интерфейс чтения с отслеживанием незнакомых слов, inline-переводом и огласовками.
...
Static UI и bot messages пишутся на английском. AI-перевод слова возвращается на русском языке.
...
Чтобы не получать галлюцинации и сразу подготовить интерактивный текст для чтения, обработка выполняется в три этапа:

```text
Сырой текст -> Этап 1: Адаптация -> Этап 2: Валидатор -> Этап 3: Enrichment inline-токенов -> Успех / Отказ
```
...
#### Этап 3: enrichment inline-токенов

После успешной валидации backend отправляет адаптированный текст в LLM и получает массив токенов для Mini App.
...
Backend валидирует, что склеивание всех `token.text` равно исходному `adapted_text`.
...
- Backend возвращает текст как массив inline-токенов `ArticleToken[]`.
- Каждое слово оборачивается в интерактивный элемент.
- Обычные текстовые токены сохраняют пробелы, пунктуацию и переносы строк.
- Пользователь может включить или выключить отображение огласовок.
- Время чтения не считается: тексты отличаются по размеру, поэтому эта метрика не используется для прогресса.

#### Клик по слову

При тапе на слово Mini App открывает Bottom Sheet с данными, которые уже пришли вместе со статьей. Отдельный API-запрос перевода не выполняется.
...
В Mini App снизу выезжает Bottom Sheet с переводом, транскрипцией, леммой и формой с огласовками. Транскрипция вычисляется backend-ом из `pointedText`; часть речи, причина выбора базовой формы и альтернативы не показываются.

При клике lemma добавляется в локальный список `translatedLemmas`. При завершении чтения backend сохраняет эти слова в `user_words` конкретного пользователя со статусом `learning`.
...
Во время отправки статистики Mini App показывает spinner на кнопке завершения. После успешного ответа backend приложение закрывается через Telegram WebApp API; в локальном preview вместо закрытия показывается success-сообщение.
...
| `reading_tokens` | `jsonb` | Inline-токены статьи: слова с переводом, lemma и огласовками, а также текстовые токены для пробелов и пунктуации |
...
- клик по слову показывает контекстный перевод и алгоритмическую транскрипцию из уже загруженного inline-токена;
- незнакомые слова фиксируются в БД;
- после завершения чтения пересчитывается уровень пользователя и Mini App закрывается при успешном ответе.
...
11. Добавить inline-токены статьи с переводом, алгоритмической транскрипцией, lemma и огласовками.
12. Добавить Bottom Sheet перевода без отдельного API-запроса.
13. Сохранять слова пользователя.
14. Добавить завершение чтения, spinner, закрытие Mini App после успешного ответа и пересчет MMR.
